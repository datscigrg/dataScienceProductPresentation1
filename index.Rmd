...
```
```{r message=FALSE,results = 'hide',echo = TRUE,fig.height=3}
    res<-sqldf("select substr(Date, 1, 7) as Date, (HeatProduction/4) as HeatProduction from data where Date like '2014%' order by Date")
      ggplot(res, aes(x=Date, y=HeatProduction)) + geom_bar(stat="identity") + labs(x="Year", y="Electric. power consumption in kWh")
